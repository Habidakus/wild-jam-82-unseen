...
func _select_spawn_spot(spots : Array[Vector2i], avoid_last_spot : bool) -> Vector2i:
	var count : int = 0
	while true:
		...
		if avoid_last_spot:
			...
			good_spot = dist_from_last_spot > 6
		if good_spot == true:
...
func _process(_delta: float) -> void:
	...
	while _spawned_fish.size() < _number_of_fish:
		...
		while spawn_spot == Vector2i.MAX && count < 100:
			...
			spawn_spot = _select_spawn_spot(viable_cells, count < 50)
